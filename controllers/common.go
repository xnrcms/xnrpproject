...
func (self *BaseController) Prepare() {
	...
	self.tplname 				= beego.AppConfig.String("tplname")
}
...
func (self *BaseController) auth() {
	...
	if len(arr) == 2 {
		...
		if userId > 0 {
			...
			//不需要权限检查
			noAuth := "ajaxsave/ajaxdel/table/loginin/registerin/loginout/getnodes/start/show/ajaxapisave/index/group/public/env/code/apidetail"
			isNoAuth := strings.Contains(noAuth, self.actionName)
	...
	if self.userId == 0 && (self.controllerName != "login" && self.actionName != "loginin" && self.controllerName != "register" && self.actionName != "registerin") {
...
func (self *BaseController) ajaxMsg2(msg interface{}, msgno int, url string) {
	out := make(map[string]interface{})
	out["status"] 		= msgno
	out["message"] 		= msg
	out["url"] 			= url
